...
//add_action( 'woocommerce_order_status_failed', 'wc_order_failed_sms' );
function cspd_wc_txtlcl_msg_order_failed( $order_id ){
   ...
   $checkadminsms = get_option('failed_sendadmin_check');
   $checkcustsms = get_option('failed_sendcust_check');
   
   	//Message to Customer
	if ($checkcustsms == "checked") {
    //$msgtocustomer = str_replace("ORDER_ID",$order_id,get_option('sms_to_cust'));
	 $orderid_rep_cust = str_replace("ORDER_ID",$order_id,get_option('failed_msgtxt_cust', 'Your order ORDER_ID of ORDER_TOTAL ORDER_CURRENCY on SITE_NAME has failed.'));
  ...
  cspd_send_textlocal_sms(get_option('textlocal_user'),get_option('textlocal_api'),get_option('sms_sender_id'),$order_phone,$msgtocustomer);
   ...
   if ($checkadminsms == "checked") {
	 $orderid_rep_admin = str_replace("ORDER_ID",$order_id,get_option('failed_msgtxt_admin', 'Order ORDER_ID of ORDER_TOTAL ORDER_CURRENCY on SITE_NAME has failed.'));
